    ...
    <Menu as="div" class="relative inline-block text-left">
        ...
        >
            ...
            <div class="auth-nav">
                ...
                <p class="auth-role">
                    <span
                        v-if="auth.user.roles.length > 1"
                        v-for="(role, index) in auth.user.roles"
                        :key="index"
                        class="capitalize"
                        >{{
                            index != auth.user.roles.length - 1
                                ? `${role}, `
                                : `& ${role}`
                        }}</span
                    >
        ...
        >
            <menu-items
                class="absolute right-0 mt-4 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
            >
                <div class="px-1 py-1">
                    <menu-item v-slot="{ active }">
                        ...
                        >
                            <svg
                                class="mr-2 h-5 w-5 fill-blue-400"
...
export default {
    ...
    methods: {
        logOut() {
            ...
            // fix for deploy
            this.$inertia.post(`${window.location.origin}/logout`, {
                onFinish: () => {
